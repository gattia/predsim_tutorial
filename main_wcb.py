...
# Make sure the script is in the path so that utilities can be imported
...
for case in cases:
    print(f"Processing case: {case}", flush=True)
    # Settings.    
    ###########################################################################
    # Model settings.
    ...
    print("Loading external function library...", flush=True)
    ...
    if solveProblem:
        print("Setting up optimal control problem...", flush=True) 
        #######################################################################
        ...
        #######################################################################
        # Solve problem.
        print("Starting optimization solver (this may take a while)...", flush=True)
    ...
    if analyzeResults:
        ...
        if gaitCycleSimulation == 'half':
            ...
            N_gaitCycle = N
            idxIC_c, legIC = getIdxIC_3D(GRF_opt[1,:], threshold,
                                     gaitCycleSimulation)
            idxIC_s = idxIC_c + 1            
            # Joint positions.
            Qs_GC = np.zeros((nJoints, N_gaitCycle))
        ...
        if stats['success'] == True:
            print(f"Reconstructed cost: {JAll_opt[0][0]}")
            print(f"Solver reported cost: {stats['iterations']['obj'][-1]}")
            assert np.isclose(
                    JAll_opt[0][0], 
                    stats['iterations']['obj'][-1], 
                    rtol=1e-4, atol=1e-4), (
                    f"decomposition cost mismatch: reconstructed={JAll_opt[0][0]}, "
                    f"solver={stats['iterations']['obj'][-1]}")
        
        # Write motion files for visualization in OpenSim GUI.
        if writeMotionFiles:
            suffix = ""
            if legIC == "failed_reconstruction":
                suffix = "_FAILED_RECONSTRUCTION"
            ...
            numpy_to_storage(labels, data, 
                             os.path.join(pathResults, f'motion{suffix}.mot'), 
                             datatype='IK')            
        ...
        if saveOptimalTrajectories:
            ...
            if not os.path.exists(os.path.join(pathTrajectories,
                                               f'optimaltrajectories{suffix}.npy')): 
                    optimaltrajectories = {}
            else:  
